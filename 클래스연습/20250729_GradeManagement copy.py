...
class GradeManagement():
    ...
    def random_grade(self):
        if random.random() < 0.95:  # 95% 확률로 숫자
    ...
    def CreateDataFile(self, filepath):
        ...
        if inputfile != '' and inputfile != None:
            filepath = inputfile
        test_data = [(fake.name(), self.random_grade(), self.random_grade(), self.random_grade()) for i in range(30)]
        with open(filepath, 'w', encoding='utf-8', newline='') as file:
            csv_writer = csv.writer(file) 

            csv_writer.writerow(['이름', '수학', '영어', '과학'])
    ...
    def GetAverages(self, filepath):
        data_list = self.DataLoading(filepath)
        # 2. 통계분석-a
    ...
    def GetMinMax(self, filepath):
        ...
        data_list_average = self.GetAverages(filepath)
